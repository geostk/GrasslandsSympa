...
for id_ in ID:
    try:
        ...
        dICL = sp.diff(ICL)/ICL[:-1]*100
        dICL = dICL[sp.isnan(dICL)==False]
        plt.figure()
        plt.plot(C[1:len(dICL)+1],dICL,'-*')
        plt.title('ID : {}'.format(id_))
        plt.axis([1,19,0,3])
        plt.savefig("/tmp/icl_{}.pdf".format(id_),dpi=300)
        t = sp.where(dICL<TH)[0]
        if len(t) <1:
            t = [len(dICL)+1]
        # print("Number of class {} for ID {}".format(t[0],id_))
        RES.append([id_,C[t[0]+1]])
    except IOError:
        None
...
def ClusterGrasslands(id_,DB,bands,C):
    ...
    print("Process Grasslands {0}, number of samples {1}, number of classes".format(id_,X.shape[0],C))
    # Run HDDA
    models, icl = [], []
    for rep in xrange(5): # Do several init 
        param = {'th':0.1,'tol':0.0001,'random_state':rep,'C':C}
